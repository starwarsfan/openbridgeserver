<template>
  <div class="flex flex-col gap-5">
    ...
    <form class="grid grid-cols-1 lg:grid-cols-[minmax(14rem,1fr)_12rem_12rem_minmax(16rem,1fr)_auto] gap-2" @submit.prevent="applyFilters">
      <QuickFilterInput
        v-model="filters.q"
        testid="knx-devices-search"
        :placeholder="t('knxDevices.searchPlaceholder')"
        :clear-label="t('common.clear')"
        @update:model-value="scheduleQueryFilter"
      />
      <input
...
function scheduleQueryFilter() {
  clearTimeout(queryFilterTimer)
  queryFilterTimer = setTimeout(applyFilters, 250)
}
