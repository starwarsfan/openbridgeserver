...
async def test_record_path_is_append_only_when_retention_is_disabled():
    ...
    try:
        ...
        original_executemany = rb._conn.executemany
        ...
        rb._conn.execute = _record_execute  # type: ignore[method-assign]
        rb._conn.executemany = _record_executemany  # type: ignore[method-assign]
...
async def test_concurrent_record_calls_are_serialized_by_single_instance_writer_lock():
    ...
    try:
        ...
        rb._record_locked = _record_locked_with_probe  # type: ignore[method-assign]
...
async def test_file_storage_uses_wal_journal_mode_for_reader_writer_concurrency(tmp_path: Path):
    ...
    try:
        assert rb._conn is not None
        async with rb._conn.execute("PRAGMA journal_mode") as cur:
